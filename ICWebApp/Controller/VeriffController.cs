    ...
    public class VeriffController : ControllerBase
    {
        ...
        public async Task<IActionResult> ResultHook()
        {
            ...
            if (result != null)
            {
                ...
                {
                    response.Firstname = response.Firstname.ToLower().Replace(" name", "");
                    response.Firstname = response.Firstname.ToLower().Replace("name ", "");
                    response.Firstname = response.Firstname.ToLower().Replace("vorname ", "");
                    response.Firstname = response.Firstname.ToLower().Replace(" vorname", "");
                }

                if (response.Lastname != null)
                {
                    response.Lastname = response.Lastname.ToLower().Replace(" name", "");
                    response.Lastname = response.Lastname.ToLower().Replace("name ", "");
                    response.Lastname = response.Lastname.ToLower().Replace("vorname ", "");
                    response.Lastname = response.Lastname.ToLower().Replace(" vorname", "");
                }
