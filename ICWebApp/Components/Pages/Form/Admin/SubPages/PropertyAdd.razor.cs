...
namespace ICWebApp.Components.Pages.Form.Admin.SubPages
{
    public partial class PropertyAdd
    {
        ...
        private FORM_Definition_Property Data { get; set; }
        ...
        protected override async Task OnInitializedAsync()
        {
            ...
            if (ID == "New")
            {
                Data = new FORM_Definition_Property();
                Data.ID = Guid.NewGuid();
                Data.FORM_Definition_ID = Guid.Parse(DefinitionID);

                await FormDefinitionProvider.SetDefinitionProperty(Data);

                if (Languages != null)
                {
                    foreach (var l in Languages)
                    {
                        if (Data.FORM_Definition_Property_Extended == null)
                        {
                            Data.FORM_Definition_Property_Extended = new List<FORM_Definition_Property_Extended>();
                        }

                        if (Data.FORM_Definition_Property_Extended.FirstOrDefault(p => p.LANG_Languages_ID == l.ID) == null)
                        {
                            var dataE = new FORM_Definition_Property_Extended()
                            {
                                FORM_Definition_Property_ID = Data.ID,
                                LANG_Languages_ID = l.ID
                            };

                            await FormDefinitionProvider.SetDefinitionPropertyExtended(dataE);
                            Data.FORM_Definition_Property_Extended.Add(dataE);
                        }
                    }
                }

                Data = await FormDefinitionProvider.GetDefinitionProperty(Data.ID);

                var count = await FormDefinitionProvider.GetDefinitionPropertyList(Guid.Parse(DefinitionID));
            ...
            {
                Data = await FormDefinitionProvider.GetDefinitionProperty(Guid.Parse(ID));

                if (Data != null && Languages != null && Data.FORM_Definition_Property_Extended != null &&
                    Data.FORM_Definition_Property_Extended.Count < Languages.Count)
                {
                    foreach (var l in Languages)
                    {
                        if (Data.FORM_Definition_Property_Extended.FirstOrDefault(p => p.LANG_Languages_ID == l.ID) == null)
        ...
        private async void SaveForm()
        {
            ...
            foreach (var e in Data.FORM_Definition_Property_Extended)
            {
                await FormDefinitionProvider.SetDefinitionPropertyExtended(e);
            }
