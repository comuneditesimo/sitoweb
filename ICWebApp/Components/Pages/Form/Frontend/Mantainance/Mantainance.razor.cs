...
using ICWebApp.Components.Components.FormRenderer;
       ...
       new List<IEditorTool>()
       {
        ...
        protected override async Task OnParametersSetAsync()
        {
            ...
            if (IsHomepage)
            {
                ...
                {
                    var Authority = await AuthProvider.GetAuthority(Definition.AUTH_Authority_ID.Value);
                    ...
                    if (Authority != null)
                    {
                        CrumbService.AddBreadCrumb("/Hp/Type/Services/" + Authority.ID, null, null, Authority.Description);
                    }
                }
            }
            else
                ...
                if (tasksToCreate != null)
                {
                    foreach (var task in tasksToCreate)
                    {
                        ...
                        if (responsible != null)
                        {
                            ...
                            {
                                var user = await AuthProvider.GetMunicipalUser(resp.AUTH_Municipal_Users_ID.Value);
                        ...
                        var newTask = await TaskService.CreateTask(2, Data.ID.ToString(), SessionWrapper.CurrentMunicipalUser.ID, notifyCreator, task.ID.ToString(), task.Description, NavManager.BaseUri + "/Backend/Form/Detail/" + Data.ID, deadline, authUsers);
                        ...
                        if (newTask != null && eskalations != null && eskalations.Count() > 0)
                        {
                            foreach (var esk in eskalations)
                            {
                                if (esk.FORM_Definition_Deadlines_TimeType_ID != null && esk.AdditionalDays != null)
                                {
                                    ...
                                    {
                                        await TaskService.CreateEskalation(newTask.ID, resultDate.Value, defTargets.Where(p => p.AUTH_Municpal_Users_ID != null).Select(p => p.AUTH_Municpal_Users_ID.Value).ToList());
                                    }
                                }
                            }
                        }
        ...
        private async void SetGeoLocation()
        {
            ...
            if (!string.IsNullOrEmpty(addressToGeocode))
            {
                ...
                {
                    Data.Mantainance_Lan = result.Longitude.ToString();
                    Data.Mantainance_Lat = result.Latitude.ToString();
        ...
        private void GeoService_OnResponse()
        {
            ...
            if (Data != null && location != null)
            {
                ...
                if (location.Lan != null)
                {
                    StartLan = location.Lan.Value;
                }
                if (location.Lat != null)
                {
                    StartLat = location.Lat.Value;
                }
        ...
        private async Task<string> SetAdress()
        {
            if (Data != null && !string.IsNullOrEmpty(Data.Mantainance_Lat) && !string.IsNullOrEmpty(Data.Mantainance_Lan))
            {
                try
                {
                    var result = await GeoService.GetAddress(new Location(double.Parse(Data.Mantainance_Lat), double.Parse(Data.Mantainance_Lan)));
