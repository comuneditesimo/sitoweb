...
namespace ICWebApp.Components.Pages.Form.Backend
{
    public partial class Mantainance
    {
        ...
        protected override async Task OnInitializedAsync()
        {
            ...
            {
                SelectedUserID = SessionWrapper.CurrentMunicipalUser.ID;
            }
            ...
            if (Definition != null)
            {
                ...
                if (tasksToCreate != null)
                {
                    foreach (var task in tasksToCreate)
                    {
                        ...
                        var contextName = Data.Mantainance_Title + " - " + SessionWrapper.CurrentMunicipalUser.FullnameComposed;
                        var notifyCreator = Data.IsMunicipal || (Data.IsMunicipalCommittee ?? false);
                        var newTask = await TaskService.CreateTask(2, Data.ID.ToString(), SessionWrapper.CurrentMunicipalUser.ID,
                                ...
                                if (esk.FORM_Definition_Deadlines_TimeType_ID != null && esk.AdditionalDays != null)
                                {
                                    ...
                                    if (resultDate != null && defTargets != null && defTargets.Count() > 0)
                                    {
                                        await TaskService.CreateEskalation(newTask.ID, resultDate.Value,
                                            defTargets.Where(p => p.AUTH_Municpal_Users_ID != null)
                                                .Select(p => p.AUTH_Municpal_Users_ID.Value).ToList());
                                    }
                                }
                            }
                        }
                ...
                if (Data.TASK_Task_ID != null)
                {
                    ...
                    if (existingTask == null)
                    {
                        ...
                        existingTask = await TaskService.CreateTask(2, Data.ID.ToString(),
                            SessionWrapper.CurrentMunicipalUser.ID, notifyCreator, Definition.AUTH_Authority_ID.ToString(),
                    ...
                    else
                    {
                        if (Data.MunicipalDeadline != null)
                        {
                            ...
                            existingTask.ContextName = Data.Mantainance_Title + " - " + SessionWrapper.CurrentMunicipalUser.FullnameComposed;;
        ...
        private async Task<bool> ResponsibleQuickAdd(TASK_Task_Responsible Responsible)
        {
            if (Data != null && Data.TASK_Task_ID != null && Definition != null &&
                Definition.AUTH_Authority_ID != null && Responsible.AUTH_Municipal_Users_ID != null)
            {
                var existingTask = await TaskProvider.GetTask(Data.TASK_Task_ID.Value);
                ...
                if (existingTask == null)
                {
                    ...
                    var contextName = Data.Mantainance_Title + " - " + SessionWrapper.CurrentMunicipalUser.FullnameComposed;
                    var notifyCreator = Data.IsMunicipal || (Data.IsMunicipalCommittee ?? false);
                    existingTask = await TaskService.CreateTask(2, Data.ID.ToString(), SessionWrapper.CurrentMunicipalUser.ID,
        ...
        {
            return Data.Mantainance_Title + " - " + SessionWrapper.CurrentMunicipalUser.FullnameComposed;
        }
    }
}
