    ...
    public partial class Application
    {
                ...
                if (Data.FORM_Definition_ID != null)
                {
                    ...
                    if(tasksToCreate != null)
                    {
                        foreach(var task in tasksToCreate)
                        {
                            ...
                            var newTask = await TaskService.CreateTask(1, Data.ID.ToString(), SessionWrapper.CurrentMunicipalUser.ID, false, task.ID.ToString(), task.Description, NavManager.BaseUri + "/Backend/Form/Detail/" + Data.ID, deadline, authUsers);
        ...
        private async void OnFirstStepChanged()
        {
            if (SelectedUserID != null && DefinitionID != null)
            {
                ...
                {
                    AUTH_Users_Anagrafic? dbUser = await AuthProvider.GetAnagraficByUserID(SelectedUserID.Value);
