...
namespace ICWebApp.Components.Pages.Form.Backend
{
    public partial class Detail : IDisposable
    {
        ...
        private bool _firstRenderWizard = false;
        ...
        private bool AttachOriginalDoc = false;
        private Guid UserLanguage = LanguageSettings.German;
        ...
        private async Task UpdateUnreadChatStatus()
        {
            ...
            FORM_Application? currentApplication = CurrentApplication;
            ...
            if (unreadMessages != null && unreadMessages.Any() && currentApplication != null && unreadMessages.Where(p => !string.IsNullOrEmpty(p.ContextElementId))
                                                                                                                          .Select(p => p.ContextElementId.ToLower().Trim()).Distinct().ToList().Contains(currentApplication.ID.ToString().ToLower().Trim()))
            {
                currentApplication.HasUnreadChatMessages = true;
            }
            else if (currentApplication != null)
            {
                currentApplication.HasUnreadChatMessages = false;
            }
        ...
        private async Task UpdateChatStep()
        {
            if (_firstRender && _firstRenderWizard && JSRuntime != null && CurrentApplication != null)
            {
                await Task.Delay(100);
                await JSRuntime.InvokeVoidAsync("MarkStepAsUnread", CurrentApplication.HasUnreadChatMessages);
            }
        }
            ...
            if (CurrentApplication != null)
            {
                ...
                if (status != null)
                {
                    ...
                    else
                    {
                        ...
                        StatusLog.AUTH_Users_ID = SessionWrapper.CurrentMunicipalUser.ID;
                        ...
                        if (Languages != null)
                        {
                            foreach (var l in Languages)
                            {
                                ...
                                if (l.ID == Guid.Parse("b97f0849-fa25-4cd0-8c7b-43f90fbe4075"))  //DE
                                {
                                    ...
                                    if (!string.IsNullOrEmpty(TextDocumentItem.German) && CurrentApplication.AUTH_Users_ID != null)
                                    {
                                        TextDocumentItem.German = await TextProvider.ReplaceGeneralKeyWords(CurrentApplication.AUTH_Users_ID.Value, TextDocumentItem.German);
                                        TextDocumentItem.German = await FormApplicationProvider.ReplaceKeywords(CurrentApplication.ID, LanguageSettings.Italian, TextDocumentItem.German, StartStatus);
                                    }
                                ...
                                else if (l.ID == Guid.Parse("e450421a-baff-493e-a390-71b49be6485f")) //IT
                                {
                                    ...
                                    if (!string.IsNullOrEmpty(TextDocumentItem.Italian) && CurrentApplication.AUTH_Users_ID != null)
                                    {
                                        TextDocumentItem.Italian = await TextProvider.ReplaceGeneralKeyWords(CurrentApplication.AUTH_Users_ID.Value, TextDocumentItem.Italian);
                                        TextDocumentItem.Italian = await FormApplicationProvider.ReplaceKeywords(CurrentApplication.ID, LanguageSettings.German, TextDocumentItem.Italian, StartStatus);
                                    }
                        ...
                        if (CurrentDefinition != null && CurrentApplication.FORM_Application_Status_ID != null)
                        {
                            ...
                            {
                                if (!string.IsNullOrEmpty(TextDocumentItem.German) && UserLanguage == LanguageSettings.German)
                                {
                                    FileDE = await CreateResponseFile(CurrentApplication, CurrentDefinition, Guid.Parse("b97f0849-fa25-4cd0-8c7b-43f90fbe4075"), TextProvider.Get("NOTIF_MANTAINANCE_STATUS_CHANGED_TITLE", LanguageSettings.German), AttachOriginalDoc);
                                }

                                if (!string.IsNullOrEmpty(TextDocumentItem.Italian) && UserLanguage == LanguageSettings.Italian)
                                {
                                    FileIT = await CreateResponseFile(CurrentApplication, CurrentDefinition, Guid.Parse("e450421a-baff-493e-a390-71b49be6485f"), TextProvider.Get("NOTIF_MANTAINANCE_STATUS_CHANGED_TITLE", LanguageSettings.Italian), AttachOriginalDoc);
                                }
        ...
        private async Task<FILE_FileInfo?> CreateResponseFile(FORM_Application App, FORM_Definition Def, Guid LANG_Language_ID, string FileName, bool AttachOriginalDoc = false)
        {
        ...
        private async Task<MSG_Message?> GetNewTransactionCreatedMessage(Guid transactionId)
        {
            ...
            if (msg != null && formCreatorUserId != null && CurrentDefinition != null)
            {
                ...
                {
                    row = "<tr><td style='padding-right: 40px'>{desc}</td><td valign='top' align='right'>{amount}</td></tr>";
                    row = row.Replace("{desc}", pos.Description);
                    row = row.Replace("{amount}", pos.Amount?.ToString("C"));
                    rows += row;
                }
                row = "<tr><td style='padding-right: 40px'><b>{desc}</b></td><td valign='top' align='right'><b>{amount}</b></td></tr>";
                row = row.Replace("{desc}", TextProvider.Get("PAYMENT_TOTAL_AMOUNT", lang));
                row = row.Replace("{amount}", transaction.TotalAmount?.ToString("C"));
                rows += row;
        ...
        private async void ShowChangeStatusWindow()
        {
            if (CurrentApplication != null)
            {
                ...
                UserLanguage = LanguageSettings.German;

                var userLang = await AuthProvider.GetSettings(CurrentApplication.AUTH_Users_ID.Value);

                if (userLang != null && userLang.LANG_Languages_ID == LanguageSettings.Italian)
                {
                    UserLanguage = LanguageSettings.Italian;
                }

                AttachOriginalDoc = false;
        ...
        private async Task<bool> ResponsibleQuickAdd(TASK_Task_Responsible Responsible)
        {
            if (CurrentApplication != null && CurrentApplication.TASK_Task_ID != null && CurrentDefinition != null && CurrentDefinition.AUTH_Authority_ID != null)
            {
                ...
                if (existingTask == null)
                {
                    ...
                    existingTask = await TaskService.CreateTask(TaskService.TASK_Context_ID, TaskService.ContextElementID, SessionWrapper.CurrentMunicipalUser.ID, false, CurrentDefinition.AUTH_Authority_ID.ToString(), TaskService.ContextName, NavManager.Uri, deadline, null, CurrentApplication.TASK_Task_ID);
                    await FormApplicationProvider.SetApplication(CurrentApplication);
                }
