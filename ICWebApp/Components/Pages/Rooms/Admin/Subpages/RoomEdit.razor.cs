...
using ICWebApp.Components.Components.Rooms;
...
namespace ICWebApp.Components.Pages.Rooms.Admin.Subpages
{
    public partial class RoomEdit
    {
        ...
        protected override async Task OnParametersSetAsync()
        {
            ...
            CrumbService.AddBreadCrumb("/Rooms/Service", "MAINMENU_ROOMBOOKING", null, null);
            ...
            if(Data != null)
            {
                if (string.IsNullOrEmpty(Data.RoomColor))
                {
                    Data.RoomColor = "var(--primary)";
                }
            }
        ...
        private async void MapClicked(MapClickEventArgs args)
        {
            ...
            if (Data != null && location != null)
            {
                ...
                var result = await GeoService.GetAddress(new Location(Data.Lat.Value, Data.Lng.Value));
        ...
        private async void SetGeoLocation()
        {
            ...
            if (!string.IsNullOrEmpty(addressToGeocode))
            {
               var result = await GeoService.GetCoordinates(addressToGeocode);
                ...
                {
                    Data.Lng = Convert.ToDouble(result.Longitude);
                    Data.Lat = Convert.ToDouble(result.Latitude);
            ...
            if(RoomOptionItem != null)
            {
                ...
                if(RoomOptionItem.ROOM_RoomOptions_Category_ID == ROOMOptionCategory.Inventory && RoomOptionItem.Quantity != null)
                {
                    ...
                    if(existingPositions == null || existingPositions.Count() == 0)
                    {
                        ...
                        NewItem.AUTH_Users_ID = SessionWrapper.CurrentMunicipalUser.ID;
        ...
        private void SetNewInventoryValue(V_ROOM_RoomOptions OptionItem)
        {
            ...
            NewInventoryPosition.AUTH_Users_ID = SessionWrapper.CurrentMunicipalUser.ID;

            ShowRoomInventoryPositionsQuickAdd = true;
            StateHasChanged();
        }
        private async Task SwitchEnable(V_ROOM_RoomOptions Option)
        {
            IsDataBusy = true;
            StateHasChanged();

            await RoomProvider.ToggleEnableOptions(Option.ID);
            await GetRoomOptionData();

            IsDataBusy = false;
            StateHasChanged();
        }
        private async Task SwitchDailyPayment(V_ROOM_RoomOptions Option)
        {
            IsDataBusy = true;
            StateHasChanged();

            await RoomProvider.ToggleDailyPayment(Option.ID);
            await GetRoomOptionData();
        ...
        private void OnColorChange(ColorPickerEventArgs args)
        {
            if (Data != null)
            {
                Data.RoomColor = args.CurrentValue.Hex;
            }
            StateHasChanged();
        }
    }
