...
namespace ICWebApp.Components.Pages.Rooms.Backend
{
    public partial class RoomBookingDetail
    {
        ...
        private void ShowChangeStatusWindow()
        {
            ...
            {
                StartStatus = CurrentBookingGroup.ROOM_BookingStatus_ID;
                V_ROOM_Booking_Status? _firstSelectableStatus = StatusList.OrderBy(p => p.SortOrder).FirstOrDefault(p => p.UserSelectable == true);
                if (_firstSelectableStatus != null)
                {
                    CurrentBookingGroup.ROOM_BookingStatus_ID = _firstSelectableStatus.ID;
                }
                TextItem = new TextItem();
        ...
        private void CloseChangeStatusWindow()
        {
            ...
            if (CurrentBookingGroup != null)
            {
                CurrentBookingGroup.ROOM_BookingStatus_ID = StartStatus;
            }
            StateHasChanged();
        }
