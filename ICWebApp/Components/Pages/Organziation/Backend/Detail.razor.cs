...
namespace ICWebApp.Components.Pages.Organziation.Backend
{
    public partial class Detail : IDisposable
    {
        ...
        private async Task UpdateUnreadChatStatus()
        {
            ...
            ORG_Request? currentRequest = CurrentApplication;
            ...
            if (unreadMessages != null && unreadMessages.Any() && currentRequest != null && unreadMessages.Where(p => !string.IsNullOrEmpty(p.ContextElementId))
                                                                                                                      .Select(p => p.ContextElementId.ToLower().Trim()).Distinct().ToList().Contains(currentRequest.ID.ToString().ToLower().Trim()))
            {
                currentRequest.HasUnreadChatMessages = true;
            }
            else if (currentRequest != null)
            {
                currentRequest.HasUnreadChatMessages = false;
            }

            Data = requests;
            CurrentApplication = currentRequest;
        ...
        private async Task<bool> CreateNewOrganization()
        {
            ...
            {
                var GV = await AuthProvider.GetUser(CurrentApplication.GV_FiscalNumber, null, SessionWrapper.AUTH_Municipality_ID.Value, true);
                ...
                else
                {
                    ...
                    newUser.AUTH_Municipality_ID = SessionWrapper.AUTH_Municipality_ID.Value;
            ...
            if (existingOrg == null || existingOrg.AUTH_Users_ID == null)
            {
                ...
                newOrg.AUTH_Municipality_ID = SessionWrapper.AUTH_Municipality_ID.Value;
