...
namespace ICWebApp.Components.Pages.Landing.Backend
{
    public partial class Landing : IDisposable
    {
        ...
        protected override async Task OnInitializedAsync()
        {
            ...
            if (SessionWrapper.AUTH_Municipality_ID != null)
            {
                ...
                SelectedContext = ContextList.Where(p => p != null).OrderBy(p => p.SortOrder).FirstOrDefault();
                if (SessionWrapper.CurrentMunicipalUser != null)
                {
                    UserTasks = await TaskProvider.GetUncompletedVTasksForUser(SessionWrapper.CurrentMunicipalUser.ID, LangProvider.GetCurrentLanguageID());
                }
                else
                {
                    UserTasks = new List<V_TASK_Task>();
                }
            }
        ...
        {
            UserStatistikGeneralList = (await TaskProvider.GetStatistikByUserGeneral(Language, SessionWrapper.CurrentMunicipalUser.ID)).ToList();
            UserStatistikBucketList = (await TaskProvider.GetStatistikByUserBucket(Language, SessionWrapper.CurrentMunicipalUser.ID)).ToList();
            UserStatistikStatusList = (await TaskProvider.GetStatistikByUserStatus(Language, SessionWrapper.CurrentMunicipalUser.ID)).ToList();
            UserStatistikPriorityList = (await TaskProvider.GetStatistikByUserPriority(Language, SessionWrapper.CurrentMunicipalUser.ID)).ToList();
            UserStatistikList = (await TaskProvider.GetStatistikByUser(SessionWrapper.AUTH_Municipality_ID.Value, SessionWrapper.CurrentMunicipalUser.ID, Language)).ToList();
        ...
        protected override async Task OnAfterRenderAsync(bool firstRender)
        {
            if (firstRender)
            {
                ...
                if (SessionWrapper.CurrentMunicipalUser != null)
                {
                    DashboardSettings = await SettProvider.GetDashboard(SessionWrapper.CurrentMunicipalUser.ID);
                    ...
                    if (DashboardSettings == null)
                    {
                        ...
                        DashboardSettings.AUTH_Users_ID = SessionWrapper.CurrentMunicipalUser.ID;
        ...
        private List<V_DASH_UserChat> GetChatData()
        {
            if (SessionWrapper.CurrentMunicipalUser != null)
            {
                return DashProvider.GetDashUserChat(SessionWrapper.CurrentMunicipalUser.ID);
            }
