...
namespace ICWebApp.Components.Pages.Canteen.Backend.Statistik
{
    ...
    public partial class StudentList
    {
        ...
        private List<V_CANTEEN_Statistic_Students> MovementsList = new List<V_CANTEEN_Statistic_Students>();
        ...
        protected override async Task OnInitializedAsync()
        {
            ...
            WeekDays.Add(TextProvider.Get("CANTEEN_DASHBOARD_SHORT_MO"));
            WeekDays.Add(TextProvider.Get("CANTEEN_DASHBOARD_SHORT_TUE"));
            WeekDays.Add(TextProvider.Get("CANTEEN_DASHBOARD_SHORT_WED"));
            WeekDays.Add(TextProvider.Get("CANTEEN_DASHBOARD_SHORT_THU"));
            WeekDays.Add(TextProvider.Get("CANTEEN_DASHBOARD_SHORT_FRI"));
        ...
        private async Task<bool> GetData()
        {
            if (SessionWrapper.AUTH_Municipality_ID != null)
            {
                var data = await CanteenProvider.GetStatisticStudents(SessionWrapper.AUTH_Municipality_ID.Value, LangProvider.GetCurrentLanguageID());
                ...
                if(SelectedWeekDays != null && SelectedWeekDays.Count() != 0)
                {
                    ...
                    if (SelectedWeekDays.Contains(TextProvider.Get("CANTEEN_DASHBOARD_SHORT_MO")))
                    {
                        data = data.Where(p => p.DayMo == true).ToList();
                    }
                    if (SelectedWeekDays.Contains(TextProvider.Get("CANTEEN_DASHBOARD_SHORT_TUE")))
                    {
                        data = data.Where(p => p.DayTue == true).ToList();
                    }
                    if (SelectedWeekDays.Contains(TextProvider.Get("CANTEEN_DASHBOARD_SHORT_WED")))
                    {
                        data = data.Where(p => p.DayWed == true).ToList();
                    }
                    if (SelectedWeekDays.Contains(TextProvider.Get("CANTEEN_DASHBOARD_SHORT_THU")))
                    {
                        data = data.Where(p => p.DayThu == true).ToList();
                    }
                    if (SelectedWeekDays.Contains(TextProvider.Get("CANTEEN_DASHBOARD_SHORT_FRI")))
                    {
        ...
        private async Task<bool> ExportToExcel()
        {
            ...
            foreach (var column in MovementsList.OrderBy(p => p.CanteenName).ThenBy(p => p.SchoolName).ThenBy(p => p.SchoolClass).ThenBy(p => p.FullName).ToList())
            {
                ...
                {
                    result = result + TextProvider.Get("CANTEEN_CREATE_SUBSCRIPTION_LACTOSE") + ",";
                }
                if (column.IsGlutenIntolerance)
                {
                    result = result + TextProvider.Get("CANTEEN_CREATE_SUBSCRIPTION_GLUTEN") + ",";
                }
