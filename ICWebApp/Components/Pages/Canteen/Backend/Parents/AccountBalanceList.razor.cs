...
namespace ICWebApp.Components.Pages.Canteen.Backend.Parents
{
    public partial class AccountBalanceList
    {
            ...
            if (Item != null)
            {
                ...
                if (Item.AUTH_User_ID != null && Item.MunicipalityID != null)
                {
                    ...
                    if (dbItem != null)
                    {
                        ...
                        {
                            if (subscriber.CANTEEN_Subscriber_Status_ID != CanteenStatus.Disabled)
                            {
        ...
        private async Task ReenableUser(V_CANTEEN_User user)
        {
            if (user.AUTH_User_ID != null && user.MunicipalityID != null)
            {
                BusyIndicatorService.IsBusy = true;
                StateHasChanged();
                
                var dbItem = CanteenProvider.GetCanteenUserByID(user.AUTH_User_ID.Value, user.MunicipalityID.Value);
                if (dbItem != null)
                {
                    dbItem.ServiceDisabledDate = null;
                    await CanteenProvider.SetCanteenUser(dbItem);
                }
                await GetData();
