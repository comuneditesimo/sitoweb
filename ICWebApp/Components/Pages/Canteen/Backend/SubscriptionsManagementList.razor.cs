...
namespace ICWebApp.Components.Pages.Canteen.Backend;
...
public partial class SubscriptionsManagementList
{
    ...
    private bool _oldSubsToArchiveExist = false;
    protected override async Task OnInitializedAsync()
    {
        BusyIndicatorService.IsBusy = false;
        StateHasChanged();

        if (SessionWrapper.AUTH_Municipality_ID == null) NavManager.NavigateTo("/Canteen");
        ...
        Subscribers = await CanteenProvider.GetVSubscribersByStatusID(CurrentStatus.ID, LangProvider.GetCurrentLanguageID());
        _oldSubsToArchiveExist = await CanteenProvider.SubsToArchiveExist(SessionWrapper.AUTH_Municipality_ID.Value);
        ...
        SubscribersAccepted = await CanteenProvider.GetVSubscribersByStatusID(CanteenStatus.Accepted, LangProvider.GetCurrentLanguageID());
        ...
        {
            Filter = new Administration_Filter_CanteenSubscriptions();
            Filter.Text = null;
            ...
            Filter.MinDistnanceFromSchool = null;
            Filter.MaxDistnanceFromSchool = null;
            Filter.Auth_User_ID = null;
            Filter.AUTH_Authority_ID = new List<Guid>();
            Filter.School_ID = new List<Guid>();
            Filter.SchoolYear_ID = new List<Guid>();
            Filter.Canteen_ID = new List<Guid>();
            Filter.Menu_ID = new List<Guid>();
    ...
    private async Task<List<V_CANTEEN_Subscriber>> GetData(Administration_Filter_CanteenSubscriptions? Filter)
    {
        ...
        {
            var subscriptions = await CanteenProvider.GetVSubscriptions(SessionWrapper.AUTH_Municipality_ID.Value, SessionWrapper.CurrentMunicipalUser.ID, Filter, LangProvider.GetCurrentLanguageID());
    ...
    private async void SetNewStatus()
    {
        ...
        foreach (var sub in SelectedItems)
        {
            ...
            {
                CANTEEN_Subscriber? subscription = await CanteenProvider.GetSubscriberWithoutInclude(sub.ID);
                ...
                if (subscription != null)
                {
                    ...
                    if (subscription != null)
                    {
                        ...
                        if (subscription.PreviousSubscriptionID != null && subscription.CANTEEN_Subscriber_Status_ID == CanteenStatus.Accepted)
                        {
                            ...
                            {
                                previous.CANTEEN_Subscriber_Status = null;
                        ...
                        if (subscription.CANTEEN_Subscriber_Status_ID == CanteenStatus.Accepted)
                        {
                            await CanteenProvider.CreateSubscriberCard(subscription.ID);
                        }
    ...
    private async Task ArchiveOldSubs()
    {
        if (await Dialogs.ConfirmAsync(TextProvider.Get("ARCHIVE_OLD_SUBS_ARE_YOU_SURE"), TextProvider.Get("INFORMATION")))
        {
            BusyIndicatorService.IsBusy = true;
            StateHasChanged();
            if(SessionWrapper.AUTH_Municipality_ID != null)
                await CanteenProvider.ArchiveOldSubscribers(SessionWrapper.AUTH_Municipality_ID.Value);
            SelectedItems = new List<V_CANTEEN_Subscriber>();
            BulkEnabled = false;
            Subscribers = await GetData(Filter);
            _oldSubsToArchiveExist = false;
            BusyIndicatorService.IsBusy = false;
            StateHasChanged();
        }
    }
