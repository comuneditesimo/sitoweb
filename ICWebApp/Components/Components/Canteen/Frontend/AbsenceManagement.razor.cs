...
namespace ICWebApp.Components.Components.Canteen.Frontend
{
    public partial class AbsenceManagement
    {
        ...
        private void AddAnchors(Guid TabID)
        {
            ...
            {
                int sortOrder = 0;
                foreach (var year in LatestMovements.Where(p => p.CANTEEN_Subscriber_ID == TabID && p.Date != null).OrderBy(p => p.Date.Value.Year).Select(p => p.Date.Value.Year).Distinct().ToList())
                {
                    foreach (var month in LatestMovements.Where(p => p.CANTEEN_Subscriber_ID == TabID && p.Date != null && p.Date.Value.Year == year).OrderBy(p => p.Date.Value.Month).Select(p => p.Date.Value.Month).Distinct().ToList())
                    {
                        var monthName = GetMonthName(month);
                        if (CurrentTab != null)
                        {
                            string SubID = CurrentTab.ToString() ?? "";
                            AnchorService.AddAnchor(monthName + " " + year.ToString(), SubID.Replace(" ", "").Replace("-", "").ToLower() + monthName.ToLower() + year.ToString(), sortOrder);
                            sortOrder++;
                        }
                    }
                }
