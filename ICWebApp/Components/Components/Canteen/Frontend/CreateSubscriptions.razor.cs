    ...
    public partial class CreateSubscriptions
    {
        ...
        protected override async Task OnInitializedAsync()
        {
            ...
            if (MyCivisService.Enabled == true)
            {
                ...
                CrumbService.AddBreadCrumb("/Canteen/MyCivis/Subscribe", "CANTEEN_CREATE_SUBSCRIPTION_TITLE", null, null, true);
            }
            else
            {
                CrumbService.AddBreadCrumb("/Canteen/Service", "MAINMENU_CANTEEN", null, null);
                CrumbService.AddBreadCrumb("/Canteen/Subscribe", "CANTEEN_CREATE_SUBSCRIPTION_TITLE", null, null, true);
            }
            ...
            {
                m.MealName = TextProvider.Get(m.TEXT_SystemTexts_Code);
            }
            ...
            if (FamilyID != null)
            {
                ...
                if (subList.Count() > 0)
                {
                    ...
                    {
                        if (sub.CANTEEN_School != null)
                        {
                            sub.SchoolClasses = SchoolClassList.Where(p => p.SchoolTypeID == sub.CANTEEN_School.SchoolTypeID).ToList();

                            foreach(var sc in sub.SchoolClasses)
                            {
                                sc.Name = TextProvider.Get(sc.TEXT_SystemText_Code);
                            }
                        }
            ...
            {
                PreviousSubscribers = await CanteenProvider.GetPreviousSubscriber(SessionWrapper.CurrentUser.ID, LangProvider.GetCurrentLanguageID());
            }
        ...
        private async void SchoolChanged(CANTEEN_Subscriber Data)
        {
            if (Data != null && Data.CANTEEN_School_ID != null)
            {
                if (SessionWrapper.CurrentUser.AUTH_Municipality_ID != null)
                {
                    ...
                    foreach (var sc in Data.SchoolClasses)
                    {
                        sc.Name = TextProvider.Get(sc.TEXT_SystemText_Code);
                    }
            ...
            if (valid)
            {
                ...
                foreach (var sub in Subscribers)
                {
                    ...
                    if (sub.CanteenMenuID != null)
                    {
                        ...
                        sub.MenuName = TextProvider.Get(menu.TEXT_SystemTexts_Code);
                    }
        ...
        private async void UpdateAddressData(CANTEEN_Subscriber subscriber)
        {
            if (subscriber.SelectedMunicipality != null)
            {
                ...
                if (data != null)
                {
                    ...
                    subscriber.Child_DomicileMunicipality = data.Name;
        ...
        private void ToggleButton(CANTEEN_Subscriber subscriber, bool Value, int button)
        {
            if(subscriber != null)
            {
                if(button == 1)
                {
                    subscriber.DayMo = Value;
                    StateHasChanged();
                }
                else if (button == 2)
                {
                    subscriber.DayTue = Value;
                    StateHasChanged();
                }
                else if (button == 3)
                {
                    subscriber.DayWed = Value;
                    StateHasChanged();
                }
                else if (button == 4)
                {
                    subscriber.DayThu = Value;
                    StateHasChanged();
                }
                else if (button == 5)
                {
                    subscriber.DayFri = Value;
                    StateHasChanged();
                }
            }
        }
