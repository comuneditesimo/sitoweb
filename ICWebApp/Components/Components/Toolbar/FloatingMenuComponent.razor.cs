...
namespace ICWebApp.Components.Components.Toolbar
{
        ...
        private async void TaskService_OnContextChanged()
        {
            ...
            if (MenuVisibility && SessionWrapper.AUTH_Municipality_ID != null && TaskService.TASK_Context_ID != null && TaskService.ContextElementID != null)
            {
                ...
                if (tasks != null)
                {
                    ...
                    {
                        MyTasksCount = tasks.Where(p => p.CompletedAt == null && p.ResponsibleUser != null && p.ResponsibleUser.ToLower().Contains(SessionWrapper.CurrentMunicipalUser.ID.ToString().ToLower())).Count();
                    }
