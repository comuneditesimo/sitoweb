...
namespace ICWebApp.Components.Components.Organization
{
    public partial class EditOrganization
    {
        ...
        {
            if (GesetzlicherVertreter != null && GesetzlicherVertreter.FiscalNumber != null && SessionWrapper.AUTH_Municipality_ID != null)
            {
                var GV = await AuthProvider.GetUser(GesetzlicherVertreter.FiscalNumber, null, SessionWrapper.AUTH_Municipality_ID.Value, true);
        ...
        private async Task<DataEnvelope<META_IstatComuni>> GetRemoteMunicipalities(DropDownListReadEventArgs args)
        {
            if (MunicipalitiesList == null)
            {
                MunicipalitiesList = await MetaProvider.GetMunicipalities();
        ...
        private async void UpdateAddressData(Guid Municipality_ID)
        {
            ...
            if (data != null)
            {
                ...
                Anagrafic.DomicileMunicipality = data.Name;
        ...
        private async void GV_UpdateAddressData(Guid Municipality_ID)
        {
            ...
            if (data != null && GesetzlicherVertreter != null)
            {
                ...
                GesetzlicherVertreter.DomicileMunicipality = data.Name;
        ...
        private void BolloFreeChangedHandler()
        {
            Anagrafic.BolloFree = !Anagrafic.BolloFree;

            if (Anagrafic.BolloFree == false || Anagrafic.BolloFree == null)
            {
        ...
        private void Genderchanged(int gender)
        {
            if(gender == 0)
            {
                MaleSelected = true;
