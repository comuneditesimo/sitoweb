...
namespace ICWebApp.Components.Components.Form.Notes
{
    public partial class FormNotes
    {
        ...
        private async Task<bool> SendMessage()
        {
            ...
            if (NoteToSend != null && SessionWrapper.CurrentMunicipalUser != null)
            {
                if (!string.IsNullOrEmpty(NoteToSend.Message) || FileInfoUploadList != null && FileInfoUploadList.Count() > 0)
                {
                    ...
                    NoteToSend.AUTH_Users_ID = SessionWrapper.CurrentMunicipalUser.ID;
