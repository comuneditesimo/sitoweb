...
using ICWebApp.Components.Components.Signing;
...
namespace ICWebApp.Components.Components.Templates
{
    public partial class TextTemplateInputComponent
    {
        ...
        private async void EditTemplate(Guid ID)
        {
            ...
            var ext = await TextProvider.GetTemplateExtended(ID);

            if (Languages != null && Template != null)
            {
                if (ext != null && ext.Count < Languages.Count)
                {
                    foreach (var l in Languages)
                    {
                        if (ext.FirstOrDefault(p => p.LANG_Languages_ID == l.ID) == null)
                        {
                            var dataE = new TEXT_Template_Extended()
                            {
                                ID = Guid.NewGuid(),
        ...
        private async void SaveTemplate()
        {
            if (Template != null && TemplateExtended != null)
            {
                ...
                {
                    extended.Content = extended.Content.Replace(" class=\"ProseMirror-trailingBreak\"", "");
