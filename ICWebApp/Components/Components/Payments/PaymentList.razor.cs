...
namespace ICWebApp.Components.Components.Payments
{
    public partial class PaymentList
    {
        ...
        private string? GetIdentifierTextCodeFromTransaction(PAY_Transaction transaction)
        {
            var identString = PayProvider.GetPagoPaIdentifierByTransaction(transaction);
            var ident = PagoPaIdentifiers.FirstOrDefault(e => e.PagoPA_Identifier == identString);
            return ident?.TEXT_SystemText_Code;
        }
    }
}
