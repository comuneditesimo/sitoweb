    ...
    public partial class CalendarComponent
    {
        ...
        protected override async Task OnInitializedAsync()
        {
            ...
            if(RoomBookingHelper != null && RoomBookingHelper.TimeFilter != null && RoomBookingHelper.TimeFilter.ShowSeries == false
                && RoomBookingHelper.TimeFilter.Meetings != null && RoomBookingHelper.TimeFilter.Meetings.Any())
            {
                foreach (TimeFilter_Meeting _meeting in RoomBookingHelper.TimeFilter.Meetings)
                {
                    if (_meeting.From_Combined != null && _meeting.To_Combined != null)
                    {
                        AppointmentData myAppointment = new AppointmentData()
                        {
                            IsReadonly = false,
                            Description = "",
                            StartTime = _meeting.From_Combined.Value,
                            EndTime = _meeting.To_Combined.Value,
                            CssClass = "scheduler-user-appointment",
                            Subject = TextProvider.Get("ROOM_SCHEDULER_DEFAULT_APPOINTMENT_TITLE")
                        };
                        if (myAppointment.StartTime.DayOfYear != myAppointment.EndTime.DayOfYear)
                        {
                            myAppointment.AppointmentString = "";
                        }
                        else
                        {
                            myAppointment.AppointmentString = myAppointment.StartTime.ToString("HH:mm") + " - " + myAppointment.EndTime.ToString("HH:mm");
                        }

                        ExistingAppointments.Add(myAppointment);
                    }
                }
            }
        ...
        private async void OnActionCompleted(AppointmentData record)
        {
            if (record != null) 
            {
                ...
                RoomBookingHelper.TimeFilter.AddNewMeeting(record.StartTime, record.StartTime, record.EndTime, record.EndTime);
            }
